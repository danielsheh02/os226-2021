...
void irq_disable(void) {
    ...
    sigaddset(&struct_sigset, SIGALRM);
    sigprocmask(SIG_BLOCK, &struct_sigset, NULL);
}

void irq_enable(void) {
    sigset_t struct_sigset;
    sigaddset(&struct_sigset, SIGALRM);
    sigprocmask(SIG_UNBLOCK, &struct_sigset, NULL);
}
...
long sched_gettime(void) {
    int time_global = time;
    int timer1 = timer_cnt();
    int timer2 = timer_cnt();

    if (timer1 <= timer2) {
        return time_global + timer2 / 1000;
    } else {
        return time_global + timer1 / 1000;
    }
}
